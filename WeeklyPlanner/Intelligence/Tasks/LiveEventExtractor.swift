...
final class LiveEventExtractor: EventExtractor {
                 ...
                 body: String) async throws -> ExtractedEvent
    {
            ...
            if #available(iOS 26.0, *) {
                return try await extractWithFoundationModels(subject: subject,
                                                             snippet: snippet,
                                                             fromName: fromName,
                                                             fromEmail: fromEmail,
                                                             body: body)
        ...
        #endif
        return ExtractedEvent(isEvent: false, title: "", startISO: "", endISO: "",
                              location: "", categoryHint: "", confidence: 0)
    }

    #if canImport(FoundationModels)
        @available(iOS 26.0, *)
        private func extractWithFoundationModels(subject: String,
                                                 snippet: String,
                                                 fromName: String,
                                                 fromEmail: String,
                                                 body: String) async throws -> ExtractedEvent
