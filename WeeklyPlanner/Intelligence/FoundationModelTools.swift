import Foundation
    ...
    struct FoundationFindEventsTool: Tool {
        ...
        func call(arguments: Arguments) async throws -> String {
            ...
            return try await MainActor.run { () -> Task<String, Error> in
                ...
                let start = calendar.date(byAdding: .day, value: arguments.startDayOffset,
                                          to: calendar.startOfDay(for: now)) ?? now
                let end = calendar.date(byAdding: .day, value: arguments.endDayOffset + 1,
                                        to: calendar.startOfDay(for: now)) ?? now
                let categories = arguments.category.flatMap { Category(rawValue: $0.lowercased()) }.map { [$0] }
                let keywords = arguments.keyword.map { [$0] } ?? []

                let query = EventQuery(dateRange: start ... end,
    ...
    struct FoundationFindFreeSlotsTool: Tool {
        ...
        func call(arguments: Arguments) async throws -> String {
            ...
            return try await Task { @MainActor in
                ...
                let end = calendar.date(byAdding: .day, value: arguments.endDayOffset + 1,
                                        to: calendar.startOfDay(for: now)) ?? now
                let slots = try await tool.run(dateRange: start ... end,
    ...
    struct FoundationScanInboxTool: Tool {
        ...
        func call(arguments: Arguments) async throws -> String {
            ...
            let tool = tool
            return try await Task { @MainActor in
                let results = try await tool.run(weekOffset: arguments.weekOffset,
                                                 today: now,
