...
enum NotificationCategoryIDs {
    ...
    static func all() -> Set<UNNotificationCategory> {
        let view = UNNotificationAction(identifier: Action.viewEvent,
                                        title: String(localized: "View"),
                                        options: [.foreground])
        let snooze = UNNotificationAction(identifier: Action.snooze10,
        ...
        let viewTask = UNNotificationAction(identifier: Action.viewTask,
                                            title: String(localized: "View"),
                                            options: [.foreground])
        let markDone = UNNotificationAction(identifier: Action.markDone,
