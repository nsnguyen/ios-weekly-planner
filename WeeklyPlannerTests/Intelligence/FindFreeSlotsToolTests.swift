...
final class FindFreeSlotsToolTests: XCTestCase {
    ...
    func testFindsThirtyMinuteSlotBeforeFirstEvent() async throws {
        ...
        let slots = try await tool.run(dateRange: dayStart ... Self.may16_2026(hour: 18),
                                       minMinutes: 30,
                                       dayPart: .morning)
        XCTAssertFalse(slots.isEmpty)
        XCTAssertTrue(slots.allSatisfy { $0.end.timeIntervalSince($0.start) >= 30 * 60 })
        XCTAssertTrue(try XCTUnwrap(slots.first?.start) >= dayStart)
        XCTAssertTrue(try XCTUnwrap(slots.first?.end) <= firstEventStart)
    }
    ...
    func testEmptyDayProducesOneSlotSpanningRange() async throws {
        ...
        let tool = FindFreeSlotsTool(store: store)
        let slots = try await tool.run(dateRange: dayStart ... dayEnd,
                                       minMinutes: 30,
                                       dayPart: .any)
    ...
    func testCapsAtFiveResults() async throws {
        ...
        let tool = FindFreeSlotsTool(store: store)
        let slots = try await tool.run(dateRange: dayStart ... dayStart.addingTimeInterval(12 * 60 * 60),
                                       minMinutes: 15,
                                       dayPart: .any)
