...
final class AvailabilityTests: XCTestCase {
    ...
    func testFallbackMessagesAreHonestAndUseProductVoice() {
        XCTAssertEqual(AvailabilityState.unavailable(.deviceNotEligible).fallbackMessage,
                       "On-device AI isn't available on this device. Showing canned suggestions.")
        XCTAssertEqual(AvailabilityState.unavailable(.modelNotReady).fallbackMessage,
                       "On-device AI is still warming up. Showing canned suggestions.")
        XCTAssertEqual(AvailabilityState.unavailable(.userDisabled).fallbackMessage,
        ...
        XCTAssertEqual(AvailabilityState.unavailable(.appleIntelligenceNotEnabled).fallbackMessage,
