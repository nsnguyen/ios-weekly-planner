...
final class LocationReminderManagerTests: XCTestCase {
    ...
    func testRegisterUpTo20Regions() {
        for i in 0 ..< 25 {
            let id = UUID()
            _ = manager.register(eventID: id,
                                 reminder: LocationReminder(name: "P\(i)", latitude: 0, longitude: 0,
                                                            radiusMeters: 150),
    ...
    func testPriorityKeepsTodayEventsActive() {
        ...
        for i in 0 ..< 20 {
            ...
            farIDs.append(id)
            _ = manager.register(eventID: id,
                                 reminder: LocationReminder(name: "Far\(i)", latitude: 0, longitude: 0,
                                                            radiusMeters: 150),
                                 content: { UNMutableNotificationContent() },
                                 proximityInDays: 7)
        }
        ...
        let todayID = UUID()
        _ = manager.register(eventID: todayID,
    ...
    func testRegionEntryFiresNotification() async throws {
        let id = UUID()
        _ = manager.register(eventID: id,
                             reminder: LocationReminder(name: "Marina", latitude: 37.8, longitude: -122.4,
                                                        radiusMeters: 150),
                             content: {
    ...
    func testEscalateToAlwaysAuthOnFarFutureEvent() {
        ...
        _ = manager.register(eventID: UUID(),
