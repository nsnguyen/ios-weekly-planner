...
final class AIStickyStackTests: XCTestCase {
    ...
    func testTapCallback_invokedWithTopInsight() async {
        ...
        let stack = AIStickyStack(insights: [top, insight(.weather, "W")],
                                  onTap: { tapped = $0
                                      exp.fulfill()
                                  },
    ...
    func testDismissCallback_invokedWithTopInsight() async {
        ...
        let stack = AIStickyStack(insights: [top],
                                  onTap: { _ in },
    ...
    func testRefreshCallback_invokedDirectly() async {
        ...
        let stack = AIStickyStack(insights: [insight(.travel, "T")],
                                  onTap: { _ in }, onDismiss: { _ in },
    ...
    func testCurrentIndex_defaultsToZero() {
        let stack = AIStickyStack(insights: [insight(.travel, "T"), insight(.weather, "W")],
                                  onTap: { _ in }, onDismiss: { _ in },
                                  onRefresh: {}, onNavigate: { _ in })
    ...
    func testNavigateCallback_invokedWithIndex() async {
        ...
        var navigatedTo: Int?
        let stack = AIStickyStack(insights: [insight(.travel, "T"), insight(.weather, "W"), insight(.keyword, "K")],
                                  onTap: { _ in }, onDismiss: { _ in },
                                  onRefresh: {}, onNavigate: { idx in navigatedTo = idx
                                      exp.fulfill()
                                  })
        stack.onNavigate(1)
        await fulfillment(of: [exp], timeout: 0.5)
